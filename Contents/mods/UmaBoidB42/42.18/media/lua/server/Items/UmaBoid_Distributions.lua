...
local function insertTableSafe(listName, pairTable)
    ...
    if not bucket or type(bucket.items) ~= "table" then
        if not missingDistroLists[listName] then
            missingDistroLists[listName] = true
            print("[UmaBoid] distribution list missing, skipped: " .. tostring(listName))
        end
        return
    end
